...
import InputPopup from "@/app/components/input-popup";
import {AlbumsGreaterThan, BandsGenreCount, IncreaseParticipants, TotalAlbumsCount} from "@/api/funcs";
        ...
        <main className={styles.main}>
            ...
            <form className={styles.filters_pane} onSubmit={e => {
                ...
                <select name='filteringColumn'>{FILTERING_COLUMNS.map((e, i) => {
                    return <option key={e} value={e}>{TABLE_HEADERS[i]}</option>
                })}</select>
                <span> = </span>
                ...
                <table className={styles.data_table}>
                    ...
                    {bands[page] && bands[page].map((e, i) =>
                        <tr key={i}>
                            ...
                            <td>{DATE_FORMAT.format(e.creationDate)}</td>
                            ...
                            <td>{e.albumsCount}</td>
                            <td className={e.establishmentDate == null ? styles.empty : ''}>{e.establishmentDate ? DATE_FORMAT.format(e.establishmentDate) : ''}</td>
            ...
            <div className={styles.controls}>
                <div>
                    ...
                    <InputPopup actionName='Удалить по id' sendRequestAction={
                        async (data) => {
                            console.log(data);
                            return await DeleteMusicBand(+(data.get('id') || 0) as number);
                        }}>
                        <label>
                            Id группы<br/>
                    ...
                    <InputPopup actionName='Больше альбомов чем число' sendRequestAction={
                        async (data) => {
                            console.log(data);
                            try {
                                const res = await AlbumsGreaterThan(+(data.get('id') || 0) as number);
                                return {isError: false, message: "Результат: " + res};
                            }catch (e){
                        ...
                        <label>
                            Значение<br/>
                            <input type="number" name='id' step='1' required/>
                        </label>
                    </InputPopup>
                </div>
                ...
                <div><InputPopup actionName='Кол-во по жанру' sendRequestAction={
                    async (data) => {
                        console.log(data);
                        const res = await BandsGenreCount(data.get('genre') as MusicGenre || MusicGenre.JAZZ);
                        return {isError: false, message: "Результат: " + res};
                }}>
                    <label>
                        Жанр<br/>
                        <select name='genre'>
                            {[MusicGenre.JAZZ, MusicGenre.ROCK, MusicGenre.BRIT_POP, MusicGenre.PSYCHEDELIC_CLOUD_RAP]
                                .map((e, i) =>
                                    <option key={i} value={e}>{e}</option>
                                )}
                        </select>
                    </label>
                </InputPopup><InputPopup actionName='Кол-во альбомов' sendRequestAction={
                    async (data) => {
                        console.log(data);
                        const res = await TotalAlbumsCount();
                        return {isError: false, message: "Результат: " + res};
                    }}>
                    <label>
                        Кол-во альбомов
                    </label>
                </InputPopup><InputPopup actionName='Увеличить кол-во участников' sendRequestAction={
                    async (data) => {
                        console.log(data);
                        try {
                            const res = await IncreaseParticipants(+(data.get('id') || 0) as number);
                            return {isError: false, message: "Результат: " + res};
                        }catch (e){
                            const message = ((e as AxiosError)?.response?.data as string) || (e as AxiosError)?.message;
                            return {isError: true, message};
